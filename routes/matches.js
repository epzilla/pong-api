      ...
      if (matchInProgress) {
        ...
        Matches.create({
          ...
          .then(startedMatch => {
            ...
            return Promise.all([
              sequelize.query(
                `insert into match_key (id, match_id) values ('${hashedToken}', '${newMatch.id}')`,
                { type: sequelize.QueryTypes.INSERT }
              ),
              sequelize.query(
                `insert into games (match_id, score1, score2, game_num) values ('${newMatch.id}', ${initialScore}, ${initialScore}, 1)`,
                { type: sequelize.QueryTypes.INSERT }
              )
            ]);
          })
...
exports.addDevices = (req, res) => {
  ...
  return validateMatchToken(req, res)
    .then(result => {
      if (!result) {
      ...
      let promises = devices.map(d => {
        ...
        return sequelize.query(
          `insert into match_key (id, match_id) values ('${hashedToken}', '${match.id}')`,
          { type: sequelize.QueryTypes.INSERT }
        );
      });
...
exports.finish = (req, res) => {
  ...
  return validateMatchToken(req, res)
    ...
    .then(updatedMatch => {
      ...
      sequelize.query(`delete from match_key where match_id='${finishedMatch.id}'`, {
