...
const buildStatQuery = req => {
  ...
  let orClause = [
    {
      [Op.and]: [{ player1Id: player1Id }, { player2Id: player2Id }]
    },
    {
      [Op.and]: [{ player1Id: player2Id }, { player2Id: player1Id }]
    }
  ];
  ...
  if (dateFrom & dateTo) {
    query.where = {
      [Op.and]: [
        {
          [Op.and]: [{ startTime: { [Op.gte]: dateFrom } }, { startTime: { [Op.lte]: dateTo } }]
        },
        { [Op.or]: orClause }
      ]
    };
...
exports.matchesByPlayers = (req, res) => {
  return Matches.findAll(buildStatQuery(req))
    .then(matches => {
      ...
      matches.forEach(m => {
        ...
        m.games.forEach(g => {
          ...
          if (!p1Avgs) {
            ...
            p1Avgs.avgMargin = parseFloat(
              (p1Avgs.avgPointsFor - p1Avgs.avgPointsAgainst).toFixed(2)
            );
          }
          ...
          if (!p2Avgs) {
            ...
            p2Avgs.avgMargin = parseFloat(
              (p2Avgs.avgPointsFor - p2Avgs.avgPointsAgainst).toFixed(2)
            );
          }
        });
      ...
      statPack.player1.avgPointsFor = parseFloat(
        (
          statPack.player1.pointsFor /
          (statPack.player1.gamesWon + statPack.player1.gamesLost)
        ).toFixed(2)
      );
      statPack.player2.avgPointsFor = parseFloat(
        (
          statPack.player2.pointsFor /
          (statPack.player2.gamesWon + statPack.player2.gamesLost)
        ).toFixed(2)
      );
      statPack.player1.avgPointsAgainst = parseFloat(
        (
          statPack.player1.pointsAgainst /
          (statPack.player1.gamesWon + statPack.player1.gamesLost)
        ).toFixed(2)
      );
      statPack.player2.avgPointsAgainst = parseFloat(
        (
          statPack.player2.pointsAgainst /
          (statPack.player2.gamesWon + statPack.player2.gamesLost)
        ).toFixed(2)
      );
      statPack.player1.avgMargin = parseFloat(
        (statPack.player1.avgPointsFor - statPack.player1.avgPointsAgainst).toFixed(2)
      );
      statPack.player2.avgMargin = parseFloat(
        (statPack.player2.avgPointsFor - statPack.player2.avgPointsAgainst).toFixed(2)
      );
      res.json(statPack);
    })
